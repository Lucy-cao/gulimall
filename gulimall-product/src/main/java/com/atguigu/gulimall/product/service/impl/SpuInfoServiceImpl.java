...
public class SpuInfoServiceImpl extends ServiceImpl<SpuInfoDao, SpuInfoEntity> implements SpuInfoService {
	...
	public void spuUp(Long spuId) {
		...
		//根据spu获取可搜索的商品属性
		List<ProductAttrValueEntity> productAttrEntities = productAttrValueService.list(
				Wrappers.lambdaQuery(ProductAttrValueEntity.class)
						.eq(ProductAttrValueEntity::getSpuId, spuId));
		List<Long> attrIds = productAttrEntities.stream().map(ProductAttrValueEntity::getAttrId).collect(Collectors.toList());
		List<Long> searchAttrIds = attrDao.selectSearchAttrs(attrIds);
		Set<Long> idSet = new HashSet<>(searchAttrIds);
		List<SkuEsModel.Attr> attrs = productAttrEntities.stream().filter(item -> idSet.contains(item.getAttrId()))
				.map(item -> {
					SkuEsModel.Attr attr = new SkuEsModel.Attr();
					BeanUtils.copyProperties(item, attr);
					return attr;
				}).collect(Collectors.toList());
		//通过openfeign调用仓库服务，批量获取sku的库存情况
		List<Long> skuIds = skuInfoEntities.stream().map(SkuInfoEntity::getSkuId).collect(Collectors.toList());
		R listR = wareFeignService.hasStock(skuIds);
		if(listR.getCode()!=0){
			throw new RRException("仓库服务调用失败！");
		}
		TypeReference<List<SkuHasStockVo>> typeReference = new TypeReference<List<SkuHasStockVo>>(){};
		List<SkuHasStockVo> skuHasStockVoList = listR.getData(typeReference);
		Map<Long, Boolean> hasStockMap = skuHasStockVoList.stream().collect(
		...
		//3、将数据传输到ES保存
		R response = searchFeignService.skuUp(esModels);
		if(response.getCode()==0){
			//4、上架成功，更新spu信息的上架状态
			SpuInfoEntity spuInfoEntity = new SpuInfoEntity();
